...
class Member(SoftDeletionModel):
    ...
    # cast info
    point_half = models.IntegerField(
        '30分あたりのポイント', default=3000, validators=[
            MaxValueValidator(100000), MinValueValidator(0)])
    video_point_half = models.IntegerField(
        'ビデオオーダー料金', default=3000, validators=[
...
class FavoriteTweet(models.Model):
    ...
    liker = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="tweet_favorites",
        verbose_name="ユーザー")
    tweet = models.ForeignKey(
        Tweet,
        on_delete=models.SET_NULL,
        null=True,
        related_name="tweet_likers",
        verbose_name="つぶやき")
    created_at = models.DateTimeField('作成日時', auto_now_add=True)
