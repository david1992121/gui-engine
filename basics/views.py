...
class ChoiceView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if category != "":
            subcategory_values = list(Choice.objects.filter(
                category=category).values_list('subcategory').distinct())
            return_value = []
            for sub_item in subcategory_values:
                cur_data = {}
                cur_data['name'] = sub_item[0]
                cur_data['data'] = ChoiceSerializer(
                    Choice.objects.filter(
                        category=category,
                        subcategory=sub_item[0]).order_by('order'),
                    many=True).data
...
class SettingView(mixins.UpdateModelMixin, generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            cur_user.save()
            return Response(
                MemberSerializer(cur_user).data,
                status.HTTP_200_OK)
        else:
            return Response(status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if is_shown != "":
            queryset = Gift.objects.filter(
                is_shown=True, image__isnull=False, location__isnull=False)
            return Response(
                GiftSerializer(
                    queryset,
                    many=True).data,
...
class CostPlanView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if location_id > 0:
            queryset = CostPlan.objects.filter(
                location_id=location_id).order_by('cost')
            return Response(
                CostplanSerializer(
                    queryset,
                    many=True).data,
