...
def create_room(order, cast_ids):
    ...
    new_message = "おめでとうございます♪ \n \
        ...
        それでは素敵な時間をお過ごしください。"
    location_name = order.location.name if order.location is not None else order.location_other
    date_str = order.meet_time_iso.astimezone(pytz.timezone(
        "Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
    room_title = "合流：{0} {1} キャスト{2}人".format(
        location_name, date_str, order.person)
    new_room = Room.objects.create(
        last_message=new_message,
        room_type="public",
        title=room_title,
        is_group=True,
        last_sender=Member.objects.get(
            username="system"),
        status=2)
