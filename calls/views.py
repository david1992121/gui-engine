...
class InvoiceView(mixins.ListModelMixin, generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if cur_request != "":
            ...
            if location_val > 0:
                query_set = query_set.filter(
                    Q(order__parent_location__id=location_val) | Q(gift__location__id=location_val))
        ...
        return Response({"total": total, "results": InvoiceSerializer(
            invoices, many=True).data}, status=status.HTTP_200_OK)
...
class UserInvoiceView(mixins.ListModelMixin, generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        query_set = Invoice.objects.filter(
            Q(giver=request.user) | Q(taker=request.user))
        total = query_set.count()
...
def get_invoice_total(request):
    ...
    normal_invoices = Invoice.objects.exclude(
        invoice_type='ADMIN').exclude(
        invoice_type='CHARGE').exclude(
            invoice_type='BUY').exclude(
                invoice_type='AUTO')
    use_point = normal_invoices.aggregate(
        Sum('give_amount'))['give_amount__sum']
    pay_point = normal_invoices.aggregate(
        Sum('take_amount'))['take_amount__sum']
...
def get_rank_users(request):
    ...
    if user_type == "guest":
        if date_from != "":
            time_filter &= Q(
                gave__created_at__gte=get_edge_time(
                    date_from, "from"))
        if date_to != "":
        ...
        if date_from != "":
            time_filter &= Q(
                took__created_at__gte=get_edge_time(
                    date_from, "from"))
        if date_to != "":
...
def get_ranking(request):
    ...
    if user_type == "false":
        time_filter = Q(
            gave__created_at__gte=start_time) & Q(
            gave__created_at__lt=end_time)
        ...
        time_filter = Q(
            took__created_at__gte=start_time) & Q(
            took__created_at__lt=end_time)
...
def create_order(request):
    ...
    if serializer.is_valid():
        ...
        if notify_cast > 0:
            ...
            if notify_cast == 1:
                cast_class_ids = list(
                    order.cost_plan.classes.values_list(
                        'id', flat=True))
                cast_ids = list(
                    Member.objects.filter(
                        role=0,
                        cast_class__id__in=cast_class_ids).values_list(
                        'id',
                        flat=True))
            else:
                cast_ids = list(
                    Member.objects.filter(
                        role=0, is_present=True).values_list(
                        'id', flat=True))
        ...
        meet_date = order.meet_time_iso.astimezone(
        ...
        cast_ids = list(
            Member.objects.filter(
                role=0,
                location=order.parent_location).values_list(
                'id',
                flat=True))
...
class OrderView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        return Response({"total": total, "results": OrderSerializer(
            orders, many=True).data}, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # send system message
            location_str = new_order.location.name if new_order.location is not None else new_order.location_other
            start_time_str = new_order.meet_time_iso.astimezone(pytz.timezone(
                "Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
            end_time_str = new_order.ended_predict.astimezone(pytz.timezone(
                "Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
            ...
            today_date = datetime.now().astimezone(pytz.timezone("Asia/Tokyo")).date()
            meet_date = new_order.meet_time_iso.astimezone(
                pytz.timezone("Asia/Tokyo")).date()

            # call send
            class_ids = list(
                new_order.cost_plan.classes.values_list(
                    'id', flat=True))
            cast_ids = list(
                Member.objects.filter(
                    role=0,
                    cast_class__id__in=class_ids).values_list(
                    'id',
                    flat=True))
        ...
        if serializer.is_valid():
            ...
            if old_status != new_order.status:
                if new_order.status >= 5:
                    for ongoingJoin in new_order.joins.filter(
                            is_started=True, is_ended=False, status=1):
                        ongoingJoin.is_ended = True
                        ongoingJoin.ended_at = timezone.now()
                        ongoingJoin.save()

                        message = "{}の合流は終了されました。".format(
                            ongoingJoin.user.nickname)
                        send_notice_to_room(new_order.room, message, True)

                    if new_order.room is not None:
                        # send room event
                ...
                if new_order.status == 8:
                    ...
                    cast_ids = list(
                        Member.objects.filter(
                            role=0, cast_class__id__in=class_ids).values_list(
                            'id', flat=True))
...
def cancel_order(request, id):
    ...
    if not cur_admin.is_superuser and cur_admin.location_id != cur_order.parent_location_id:
        ...
        # end join
        for ongoingJoin in cur_order.joins.filter(
                status=1, is_started=True, is_ended=False):
            ongoingJoin.is_ended = True
            ongoingJoin.ended_at = timezone.now()
            ongoingJoin.save()
        ...
        class_ids = list(
            cur_order.cost_plan.classes.values_list(
                'id', flat=True))
        cast_ids = list(
            Member.objects.filter(
                role=0,
        ...
        return Response(
            OrderSerializer(cur_order).data,
            status=status.HTTP_200_OK)
...
def get_order_counts(request):
    ...
    if is_normal == 1:
        ...
        status_title = [
            ...
            'Extended time exceeds 8 hours',
            'Payment failed']

        # collecting cast not enough
        query_set = Order.objects.annotate(
            joined_count=Count('joins')).filter(
            status=0, joined_count__lt=F('person'))
        ...
        query_set = Order.objects.filter(
            status=3, ended_predict__lt=timezone.now())
        orders_count = query_set.count()
        ids_array = list(query_set.values_list('id', flat=True).distinct())
        ...
        query_set = Order.objects.filter(
            status=4, ended_predict__lt=ended_predict_time)
        orders_count = query_set.count()
        ids_array = list(query_set.values_list('id', flat=True).distinct())
...
def get_month_data(request):
    ...
    for single_date in daterange(start_date, end_date):
        ...
        temp_data["date_str"] = "{0}({1})".format(
            single_date.strftime("%Y-%m-%d"), weekday_str[single_date.weekday()])
        temp_data["order_counts"] = Order.objects.filter(
            created_at__date=single_date).count()
        ...
        if temp_data["order_points"] is not None:
            sum_data["order_points"] += temp_data["order_points"]

        temp_data['gift_counts'] = Invoice.objects.filter(
            created_at__date=single_date, invoice_type="GIFT").count()
        ...
        if temp_data['gift_points'] is not None:
        ...
        if temp_data['buy_points'] is not None:
...
class JoinView(mixins.CreateModelMixin, generics.GenericAPIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if new_obj.order.room is not None:
                cur_room = new_obj.order.room
                if ex_started_at is None and new_obj.started_at is not None:
                    message = "{0}は合流しました。".format(new_obj.user.nickname)
                    new_obj.is_started = True
                    new_obj.save()
                    send_notice_to_room(cur_room, message)
                if ex_started_at is not None and new_obj.started_at is None:
                    message = "{0}の合流は管理者によりキャンセルされました。".format(
                        new_obj.user.nickname)
                    new_obj.is_started = False
                    new_obj.save()
                    send_notice_to_room(cur_room, message)
                if ex_ended_at is None and new_obj.ended_at is not None:
                    message = "{0}は解散しました。".format(new_obj.user.nickname)
                    new_obj.is_ended = True
                    new_obj.save()
                    send_notice_to_room(cur_room, message)
                if ex_ended_at is not None and new_obj.ended_at is None:
                    message = "{0}の解散は管理者によりキャンセルされました。".format(
                        new_obj.user.nickname)
                ...
                if not ex_ended and new_obj.is_ended:
                    if cur_order.joins.filter(
                            is_ended=False).count() == 0 and cur_order.status < 5:
                        cur_order.status = 5
                        cur_order.save()
...
class OrderCastView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        query_set = Order.objects.filter(
            cost_plan__classes__id=cur_user.cast_class_id,
            is_private=False)
        ...
        if mode == 'today':
            query_set = query_set.filter(
                status__lt=3,
                meet_time_iso__gte=today_date,
                meet_time_iso__lt=tomorrow_date)
        elif mode == 'tomorrow':
            query_set = query_set.filter(
                status__lt=3, meet_time_iso__gte=tomorrow_date)
        else:
            query_set = query_set.filter(
                status__lte=6).filter(
                joins__user=cur_user)
...
def apply_order(request, id):
    try:
        ...
        if joins_num == cur_order.person and cur_order.status == 0:
            ...
            collect_end = cur_order.collect_ended_at.astimezone(
            ...
            if range_item['ended_at'] > started_time and range_item['started_at'] < end_time:
                if range_item['allowed']:
                    cur_orders.append(
                        OrderSerializer(
                            Order.objects.get(
                                pk=range_item['id'])).data)
                else:
                    return Response({"result": False, "data": [OrderSerializer(
                        Order.objects.get(pk=range_item['id'])).data]})
        return Response({
...
def check_cast_status(request, id):
    try:
        ...
        print(start_time)
        print(start_time.tzinfo is None or start_time.tzinfo.utcoffset(
            start_time) is None)
        print(end_time)
        try:
            ...
            for joinItem in cast.joins.filter(is_ended=False):
                if joinItem.is_started:
                    if joinItem.started_at < end_time:
                        if joinItem.started_at + timedelta(hours=joinItem.order.period) > start_time:
                            return Response(False)
                else:
                    if joinItem.order.meet_time_iso < end_time and joinItem.order.meet_time_iso + \
                            timedelta(hours=joinItem.order.period) > start_time:
                        return Response(False)
...
def confirm_cast(request, id, user_id):
    ...
    try:
        ...
        if cur_order.joins.filter(user__id=cur_user.id).count() == 0:
            ...
            cur_order.joins.filter(
                user__id=cur_user.id).update(
                status=1,
                selection=1)
            room_created = False
            room_id = 0
            ...
            if cur_order.joins.filter(status=1,
                                      selection=1).count() == cur_order.person:
                cast_ids = list(
                    cur_order.joins.filter(
                        status=1,
                        selection=1).values_list(
                        'user_id',
                        flat=True))
                room_created = True
            ...
            remove_cast_ids = list(
                cur_order.joins.exclude(
                    status=1,
                    selection=1).values_list(
                    'user_id',
                    flat=True))
            start_time_str = cur_order.meet_time_iso.astimezone(pytz.timezone(
                "Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
            message = "オーダーにエントリー頂き、誠にありが \
                とうございました。 残念ながら「合流: {0} {1} キャスト{2}人」のマッチングでは外れました。\
                是非またオーダーにエントリー頂けます。ようお願いいたします!".format(
                cur_order.location.name if cur_order.location else cur_order.location_other,
                start_time_str,
                cur_order.person)
            ...
            return Response(
                {"created": room_created, "room": room_id}, status=status.HTTP_200_OK)
    except Order.DoesNotExist:
        return Response(status=status.HTTP_400_BAD_REQUEST)
...
def make_room(request, id):
    ...
    if cur_admin.location_id != cur_order.parent_location_id:
        ...
        if cur_order.room is not None:
            ...
            add_users = []
            for cast_id in cast_ids:
                if cast_id not in old_cast_ids:
...
def auto_match(request, id):
    ...
    try:
        ...
        if cur_order.joins.filter(status=1).count() < persons:
            ...
            if cur_order.joins.filter(status=0, dropped=False).count(
            ) < persons - cur_order.joins.filter(status=1).count():
                return Response(status=status.HTTP_406_NOT_ACCEPTABLE)
            ...
            candidates = list(
                cur_order.joins.filter(
                    status=0,
                    dropped=False).order_by('user__call_times').values_list(
                    'id',
                    flat=True))
            added_cast_ids = []
...
def check_meet_time(request, id):
    try:
        ...
        if room.users.filter(id=request.user.id).count() > 0:
            cur_order = room.orders.filter(is_private=not room.is_group).filter(
                Q(status=4) | Q(status=3)).order_by('-status', 'meet_time_iso').first()
            if cur_order is not None:
...
def get_room_joins(request, id):
    try:
        ...
        if room.users.filter(id=request.user.id).count() > 0:
            ...
            if cur_order is not None:
                if cur_order.joins.filter(
                        status=1, user=request.user).count() > 0:
                    cur_join = cur_order.joins.filter(
                        status=1, user=request.user).first()
                    return Response(
                        {"result": True, "data": JoinSerializer(cur_join).data})
...
def check_room_status(request, id):
    try:
        room = Room.objects.get(pk=id)
        if room.users.filter(id=request.user.id).count() > 0:
            cur_order = room.orders.filter(is_private=True).filter(
                Q(status=4) | Q(status=3)).order_by('-status', 'meet_time_iso').first()

            return Response(cur_order is None, status=status.HTTP_200_OK)
        else:
            return Response(status=status.HTTP_400_BAD_REQUEST)
    except Room.DoesNotExist:
        return Response(status=status.HTTP_400_BAD_REQUEST)
...
def start_joins(request, id):
    try:
        room = Room.objects.get(pk=id)
        # if room.status == 2:
        if room.users.filter(id=request.user.id).count(
        ) > 0 and room.orders.count() > 0:
...
def request_call(request):
    ...
    if serializer.is_valid():
        ...
        period_start_str = order.meet_time_iso.astimezone(pytz.timezone(
            "Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
        period_end_str = (
            order.meet_time_iso + timedelta(hours=order.period)).astimezone(
            pytz.timezone("Asia/Tokyo")).strftime("%Y{0}%m{1}%d{2}%H{3}%M{4}").format(*"年月日時分")
        message = ""
        ...
        if request.user.role == 0:
            message = "個人オーダーのリクエストありがとうございます。\n\
                ...
                時間：{1} ~ {2}\n\
                料金：{3:,d}P\n\
                <a href = '/main/chat/request/{4}?room_id={5}'>詳細はこちら</a>".format(order.location.name,
                                                                                  period_start_str,
                                                                                  period_end_str,
                                                                                  order.cost_value * 2 * order.period,
                                                                                  order.id,
                                                                                  order.room.id)
        else:
            message = "個人オーダーのリクエストありがとうございます。\n\
    ...
    try:
        ...
        if order.user.id == cur_user.id:
            ...
            if order.is_cancelled or order.is_replied:
                ...
                Join.objects.create(
                    user=cast, order=order, status=1, selection=0)
...
class ReviewView(
    ...
    def get_queryset(self):
        ...
        if user_id > 0:
            return Review.objects.filter(
                target_id=user_id).order_by('-created_at')
        else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({"total": total, "results": ReviewSerializer(
...
def get_reviews(request, id):
    try:
        ...
        if cur_order.reviews.filter(source=request.user).count() > 0:
            return Response({"result": True, "data": ReviewSerializer(
                cur_order.reviews.filter(source=request.user), many=True).data})
        else:
            user_ids = []
            if cur_order.is_private:
                ...
                if request.user.role == 1:
                    user_ids = list(
                        cur_order.joins.filter(
                            status=1,
                            is_ended=True).values_list(
                            'user_id',
                            flat=True))
                else:
                    user_ids = [cur_order.user.id]
            return Response({"result": False, "data": MainInfoSerializer(
                Member.objects.filter(id__in=user_ids), many=True).data})
    except Order.DoesNotExist:
        return Response(status=status.HTTP_400_BAD_REQUEST)
    ...
    if cur_query != "":
        ...
        if query_date != "":
            start_time = datetime.strptime(
                "{}-+0900".format(query_date), "%Y-%m-%d-%z")
        ...
        for user in users:
            ...
            for join in join_query.filter(
                    ended_at__gt=start_time,
                    started_at__lt=end_time).order_by("started_at"):
